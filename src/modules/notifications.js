...
export default function showNotification(state = initialState, action) {
    switch (action.type) {
        ...
        case SHOW_NOTHIFICATION_SUCCESS: {
            const { message = '' } = action.payload;
            return { ...state, ...{ show: true, message: `${message}`, type: 'success' } };
        }
        case SHOW_NOTHIFICATION_WARN: {
            const { message = '' } = action.payload;
            return { ...state, ...{ show: true, message: `${message}`, type: 'warning' } };
        }
        case SHOW_NOTHIFICATION_ERROR: {
            const { message = '' } = action.payload;
            return { ...state, ...{ show: true, message: `${message}`, type: 'error' } };
        }
        case HIDE_NOTIFICATIONS_ALL:
            return { ...state, ...{ show: false, message: initialState.message } };
        default:
            return state;
    }
}
